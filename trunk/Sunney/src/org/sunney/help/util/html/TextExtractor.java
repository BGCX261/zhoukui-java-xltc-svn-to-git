package org.sunney.help.util.html;
...
public class TextExtractor {
	...
	public void extractHTML(String htmlText) {
		...
		for (int i = 1; i < textList.size(); i++) {
			if (textBeginList.get(i) == textEndList.get(i - 1) + 1 && textEndList.get(i) > textBeginList.get(i) + _block && textList.get(i).replaceAll("\\s+", "").length() > 40) {
	...
	private List<Integer> lineBlockDistribute(List<String> lines) {
		...
		for (int i = 0; i + 4 < lines.size(); i++) {
			if (indexDistribution.get(i) == 0 && indexDistribution.get(i + 1) == 0 && indexDistribution.get(i + 2) > 0 && indexDistribution.get(i + 2) < 40 && indexDistribution.get(i + 3) == 0 && indexDistribution.get(i + 4) == 0) {
...
//		te.extractURL("http://blog.sina.com.cn/s/blog_4cbec5e90100dgyy.html");
		te.extractURL("http://zhibo8.com/index.html");
